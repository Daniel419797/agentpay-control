import type { AgentPayClient, AutonomousCardPurchaseRequest, MooveReceivePayment, MooveReceiveRequest, PaidRequest, PaymentIntent } from "../../sdk/src/index";
...
export function createAgentPayMooveReceiveTool(client: AgentPayClient, agentId: string) {
  return {
    ...
    description: "Create a Moove Receive payment link so the agent can accept payment into its organization's configured Moove settlement wallet. The payment is not complete until providerStatus is COMPLETED. Reuse idempotencyKey for retries of the same intended link.",
    schema: {
      type: "object",
      properties: {
...
export function createAgentPayTools(client: AgentPayClient, agentId: string) {
  return [
    createAgentPayTool(client, agentId),
    createAgentPayCardPurchaseTool(client, agentId),
    createAgentPayCardStatusTool(client),
    createAgentPayMooveReceiveTool(client, agentId),
  ];
}
