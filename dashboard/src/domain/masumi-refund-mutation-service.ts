...
async function authorizedEscrow(tx: Prisma.TransactionClient, escrowPurchaseId: string, organizationId: string, access: AccessMode) {
  const rows = access === "BUYER"
    ? await tx.$queryRaw<EscrowMutationRow[]>`
        SELECT "id","organizationId","network","blockchainIdentifier","state","refundRequestedAt","refundAuthorizedAt"
        FROM "MasumiEscrowPurchase"
        WHERE "id"=${escrowPurchaseId}::uuid AND "organizationId"=${organizationId}::uuid
        LIMIT 1`
    : await tx.$queryRaw<EscrowMutationRow[]>`
        SELECT p."id",p."organizationId",p."network",p."blockchainIdentifier",p."state",p."refundRequestedAt",p."refundAuthorizedAt"
  ...
  return db.$transaction(async (tx) => {
    ...
    if (locallyConfirmed(input.operation, escrow)) {
      await tx.$executeRaw`
        ...
        ON CONFLICT ("escrowPurchaseId","operation") DO UPDATE SET "status"='CONFIRMED',"requestHash"=EXCLUDED."requestHash","failureCode"=NULL,"updatedAt"=now()`;
    ...
    await tx.$executeRaw`
      ...
      ON CONFLICT ("escrowPurchaseId","operation") DO UPDATE SET "status"='PREPARED',"requestHash"=EXCLUDED."requestHash","providerEvidence"=NULL,"failureCode"=NULL,"updatedAt"=now()`;
...
async function recordConfirmed(input: { escrow: EscrowMutationRow; operation: MasumiRefundOperation; providerState: string; providerEvidence: unknown }) {
  return db.$transaction(async (tx) => {
    ...
    await tx.$executeRaw`
      ...
      ON CONFLICT ("escrowPurchaseId","operation") DO UPDATE SET "status"='CONFIRMED',"requestHash"=EXCLUDED."requestHash","providerEvidence"=EXCLUDED."providerEvidence","failureCode"=NULL,"updatedAt"=now()`;
    await tx.$executeRaw`
...
async function claimAndEscrow(escrowPurchaseId: string, operation: MasumiRefundOperation) {
  const rows = await db.$queryRaw<Array<EscrowMutationRow & { claimId: string; claimStatus: MutationStatus }>>`
    SELECT p."id",p."organizationId",p."network",p."blockchainIdentifier",p."state",p."refundRequestedAt",p."refundAuthorizedAt",
...
export async function reconcileMasumiRefundMutation(escrowPurchaseId: string, operation: MasumiRefundOperation) {
  ...
  await db.$executeRaw`
    UPDATE "MasumiEscrowPurchase"
    SET "state"=${providerState},"providerState"=${providerState},"providerEvidence"=${JSON.stringify(purchase)}::jsonb,"lastReconciledAt"=now(),"updatedAt"=now()
    WHERE "id"=${escrowPurchaseId}::uuid`;
  ...
  return { escrowPurchaseId, state: providerState, mutation: operation, mutationStatus: "SUBMISSION_UNKNOWN" as const };
}
...
async function executeMutation(input: { escrowPurchaseId: string; organizationId: string; operation: MasumiRefundOperation; access: AccessMode }) {
  ...
  try {
    ...
    return recordConfirmed({ escrow: reserved.escrow, operation: input.operation, providerState: purchase.NextAction.requestedAction, providerEvidence: purchase });
  } catch (error) {
