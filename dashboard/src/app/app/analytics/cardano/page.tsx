...
export default async function CardanoAnalyticsPage() {
  ...
  return <section style={{ padding: 24, display: "grid", gap: 20 }}>
    ...
    <article className="surface-card" style={{ padding: 16, overflowX: "auto" }}><h2>Daily activity</h2>{activity.length ? <table className="data-table mobile-card-table"><thead><tr><th>Day</th><th>Transactions</th><th>USDCx</th><th>ADA / other</th><th>Fees (ADA)</th></tr></thead><tbody>{activity.map((row, index) => <tr key={`${String(row.day ?? "day")}-${index}`}><td data-label="Day">{value(row, "day")}</td><td data-label="Transactions">{value(row, "transactions")}</td><td data-label="USDCx">{value(row, "usdcx_transactions")}</td><td data-label="ADA / other">{value(row, "non_usdcx_transactions")}</td><td data-label="Fees (ADA)">{value(row, "network_fees_ada")}</td></tr>)}</tbody></table> : <p>No completed public activity rows are available yet.</p>}</article>
