...
function mapped(error: Error) {
  ...
  if (error.message === "MOOVE_INTEGRATION_NOT_CONFIGURED") return problem(409, error.message, "Moove is not configured for the active workspace.");
  if (error.message === "MOOVE_INTEGRATION_SECRET_INVALID") return problem(503, error.message, "The stored Moove integration secret cannot be used.");
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return ok(await withMooveConfigForOrganization(identity.organizationId, async () => getMooveReceivePayment({ organizationId: identity.organizationId, id, refresh })));
  } catch (error) {
