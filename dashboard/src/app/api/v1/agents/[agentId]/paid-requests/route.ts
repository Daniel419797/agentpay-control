...
function paymentProblem(error: Error) {
  if (error.message.startsWith("PYTH_PROVIDER_") || error.message.startsWith("MASUMI_PROVIDER_") || error.message.startsWith("MASUMI_PAYMENT_INFORMATION_")) {
    return problem(503, "PAYMENT_TRUST_PROVIDER_UNAVAILABLE", "A payment-critical external trust provider is unavailable. No spend was authorized.");
  }
  ...
  const codes: Record<string, number> = {
    ...
    PAYMENT_CONTEXT_CHANGED_RETRY: 409,
    MASUMI_BINDING_CHANGED_RETRY: 409,
    ARC_ASSET_UNSUPPORTED: 409,
    EVM_ASSET_NETWORK_UNSUPPORTED: 409,
    HEDERA_TOKEN_ID_REQUIRED: 409,
    PAYMENT_NETWORK_UNSUPPORTED: 409,
    CARDANO_NETWORK_UNSUPPORTED: 409,
    CARDANO_ASSET_UNSUPPORTED: 409,
    CARDANO_PREPROD_USDCX_ASSET_ID_REQUIRED: 503,
    CARDANO_MAINNET_USDCX_ASSET_ID_REQUIRED: 503,
    ...
    PLATFORM_CARDANO_PREPROD_PAYEE_NOT_CONFIGURED: 503,
    PLATFORM_CARDANO_MAINNET_PAYEE_NOT_CONFIGURED: 503,
    ...
    PYTH_ASSET_UNSUPPORTED: 409,
    MASUMI_RESOURCE_NOT_BOUND: 409,
    MASUMI_SETTLEMENT_ADDRESS_REQUIRED: 409,
    MASUMI_POLICY_NETWORK_MISMATCH: 409,
    MASUMI_RESOURCE_NETWORK_MISMATCH: 409,
    MASUMI_AGENT_NOT_ALLOWED: 403,
    MASUMI_CAPABILITY_NOT_ALLOWED: 403,
    ...
    MASUMI_PAYMENT_INFORMATION_IDENTITY_MISMATCH: 409,
    MASUMI_PAYMENT_INFORMATION_REGISTRY_MISMATCH: 409,
    MASUMI_PAYMENT_INFORMATION_URL_MISMATCH: 409,
  };
...
export async function POST(request: Request, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  try {
    ...
    if (error instanceof Error) {
      const mapped = paymentProblem(error);
