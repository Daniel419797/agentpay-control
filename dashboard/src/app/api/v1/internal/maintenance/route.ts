...
export async function POST(request: Request) {
  try {
    ...
    const [payments, retention, deletions, resourceHealth, invoices, crossChain, fiat, scheduledAutomations, eventAutomations, deferredAutomations, contractReconciliation, intelligence] = await Promise.all([runPaymentMaintenance(), runRetentionMaintenance(), finalizeDeletionRequests(), runResourceHealthChecks(), markOverdueInvoices(), reconcileCrossChainTransfers(), reconcileUnknownFiatTransfers(), runScheduledAutomations(), runEventDrivenAutomations(), resumeDeferredAutomationPayments(), reconcileUnknownContractExecutions(), runAllFinancialIntelligence()]);
    const incidents = await openUnresolvedSubmissionIncidents();
    return ok({ payments, retention, deletions, resourceHealth, invoices, crossChain, fiat, incidents, automations: { scheduled: scheduledAutomations, eventDriven: eventAutomations, deferredPayments: deferredAutomations, contractReconciliation }, intelligence });
  } catch (error) {
