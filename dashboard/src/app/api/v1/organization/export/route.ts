...
export async function GET(request: Request) {
  try {
    ...
    const body = JSON.stringify({ schemaVersion: 3, exportedAt: new Date().toISOString(), organization, members, agents, paymentIntents: intents, providers, auditEvents, notificationEndpoints, deletionRequests, cardholders, cards, cardAuthorizations, fiatAccounts, fiatTransfers, invoices, reviews, crossChainTransfers, automationRules, automationExecutions, financialIntelligence: { observations, forecasts, anomalies, recommendations, runs: intelligenceRuns } }, (_key, value) => typeof value === "bigint" ? value.toString() : value, 2);
    return new Response(body, { headers: { "content-type": "application/json; charset=utf-8", "content-disposition": `attachment; filename="agentpay-${organization.slug}-export.json"`, "cache-control": "private, no-store" } });
  } catch (error) {
