...
async function validateEvidence(input: z.infer<typeof schema>) {
  ...
  if (cardanoCanaries.has(input.evidenceType)) {
    ...
    const payerAddress = input.evidence.payerAddress, payeeAddress = input.evidence.payeeAddress, amountAtomic = input.evidence.amountAtomic;
    if (typeof payerAddress !== "string" || typeof payeeAddress !== "string" || typeof amountAtomic !== "string" || !/^[1-9]\d*$/.test(amountAtomic)) throw new Error("CARDANO_CANARY_PAYMENT_FACTS_REQUIRED");
  ...
  if (input.evidenceType === "DUNE_SAMPLE_VERIFIED") {
    const hashes = input.evidence.blockfrostVerifiedTransactionIds;
    if (!Array.isArray(hashes) || !hashes.length || !hashes.every((hash) => typeof hash === "string" && /^[0-9a-f]{64}$/.test(hash))) throw new Error("DUNE_SAMPLE_CHAIN_EVIDENCE_REQUIRED");
  }
  if (input.evidenceType === "INDEPENDENT_SECURITY_ASSESSMENT") {
    const report = input.evidence.reportUrl, assessor = input.evidence.assessor;
