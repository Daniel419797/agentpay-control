...
import { completeLeasedCardPurchase } from "@/domain/card-autonomy-repository";
...
export async function POST(request: Request, { params }: { params: Promise<{ purchaseId: string }> }) {
  try {
    ...
    const purchase = await completeLeasedCardPurchase({ ...input, purchaseId, resultUrl: sanitizeResultUrl(input.resultUrl) });
