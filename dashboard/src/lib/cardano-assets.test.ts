...
describe("Cardano asset whitelist", () => {
  ...
  it("pins Mainnet USDCx to Circle xReserve's canonical Cardano asset", () => {
    expect(cardanoAssetConfigFromEnv({ CARDANO_MAINNET_USDCX_ASSET_ID: CARDANO_MAINNET_USDCX_ASSET_ID }).mainnetUsdcxAssetId).toBe(CARDANO_MAINNET_USDCX_ASSET_ID);
    expect(() => cardanoAssetConfigFromEnv({ CARDANO_MAINNET_USDCX_ASSET_ID: PREPROD_TEST_STABLE_UNIT })).toThrow("CARDANO_MAINNET_USDCX_ASSET_ID_MISMATCH");
