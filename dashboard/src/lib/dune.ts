...
const resultSchema = z.object({
  ...
  execution_ended_at: z.string().optional().nullable(),
  result: z.object({
    metadata: z.object({
...
export type DuneConfig = {
  apiUrl: string;
  apiKey: string;
  overviewQueryId: number;
  activityQueryId?: number;
  dashboardUrl?: string;
  timeoutMs: number;
};
...
export function duneConfigFromEnv(env: NodeJS.ProcessEnv = process.env): DuneConfig {
  const apiUrl = (env.DUNE_API_URL || "https://api.dune.com/api/v1").replace(/\/$/, "");
  ...
  return { apiUrl, apiKey, overviewQueryId, activityQueryId, dashboardUrl: env.DUNE_DASHBOARD_URL, timeoutMs };
}

export async function fetchDuneQueryResult(queryId: number, config: DuneConfig = duneConfigFromEnv()) {
  const response = await fetch(`${config.apiUrl}/query/${queryId}/results?limit=500`, {
    headers: { "X-Dune-Api-Key": config.apiKey, accept: "application/json" },
    cache: "no-store",
    redirect: "error",
...
export async function fetchAgentPayDuneAnalytics(config: DuneConfig = duneConfigFromEnv()) {
  ...
  return { overview, activity, dashboardUrl: config.dashboardUrl ?? null };
}

export function duneReadinessErrors(env: NodeJS.ProcessEnv = process.env): string[] {
  if (env.DUNE_ANALYTICS_ENABLED !== "true") return [];
  try {
    duneConfigFromEnv(env);
    return [];
  } catch (error) {
    return [error instanceof Error ? error.message : "DUNE_CONFIG_INVALID"];
  }
}
