...
const purchaseSchema = z.object({
  ...
  resultHash: z.string().optional().nullable(),
  submitResultTime: z.union([z.string(), z.number()]).optional().nullable(),
  unlockTime: z.union([z.string(), z.number()]).optional().nullable(),
  externalDisputeUnlockTime: z.union([z.string(), z.number()]).optional().nullable(),
  NextAction: nextActionSchema,
  PaidFunds: z.array(z.object({ amount: z.string().regex(/^\d+$/), unit: z.string() }).passthrough()).default([]),
  PaymentSource: z.object({ network: z.enum(["Preprod", "Mainnet"]), paymentType: z.string(), policyId: z.string().optional(), smartContractAddress: z.string().optional() }).passthrough(),
  CurrentTransaction: z.unknown().optional().nullable(),
}).passthrough();
...
const startJobSchema = z.object({
  ...
  sellerVKey: z.string().regex(/^[0-9a-fA-F]{56}$/),
  identifierFromPurchaser: z.string().min(14).max(26),
  input_hash: z.string().regex(/^[0-9a-fA-F]{64}$/),
}).passthrough();
const statusSchema = z.object({
  status: z.enum(["awaiting_payment", "awaiting_input", "running", "completed", "failed"]),
...
export type MasumiPaymentConfig = {
  baseUrl: string;
  apiKey: string;
  timeoutMs: number;
...
export function masumiInputHash(input: unknown) {
  return createHash("sha256").update(stable(input)).digest("hex");
}

export function masumiResultHash(result: unknown) {
  return createHash("sha256").update(typeof result === "string" ? result : stable(result)).digest("hex");
}

export function masumiPaymentConfigFromEnv(env: NodeJS.ProcessEnv = process.env): MasumiPaymentConfig {
  const baseUrl = (env.MASUMI_PAYMENT_URL || "").replace(/\/$/, "");
  const apiKey = env.MASUMI_PAYMENT_API_KEY || "";
  const timeoutMs = Number(env.MASUMI_PAYMENT_TIMEOUT_MS || "10000");
  const maxLookupPages = Number(env.MASUMI_PAYMENT_MAX_LOOKUP_PAGES || "10");
...
function headers(config: MasumiPaymentConfig, json = false) {
  return { accept: "application/json", token: config.apiKey, ...(json ? { "content-type": "application/json" } : {}) };
}
...
async function providerJson(url: string | URL, config: MasumiPaymentConfig, init: RequestInit = {}) {
  ...
  if (!response.ok) {
    ...
    // A timeout/network failure after a money-moving POST is handled by callers as
    // ambiguous. A received 5xx/409 is also not proof that no chain transaction exists.
    error.ambiguous = response.status >= 500 || response.status === 409 || response.status === 408 || response.status === 429;
    throw error;
  }
  return payload;
}
...
export async function startMasumiJob(entry: MasumiVerifiedEntry, inputData: unknown, identifierFromPurchaser = randomBytes(10).toString("hex")): Promise<MasumiStartJob> {
  ...
  if (process.env.APP_ENV === "production" && base.protocol !== "https:") throw new Error("MASUMI_AGENT_HTTPS_REQUIRED");
  const response = await fetch(new URL("start_job", base.href.endsWith("/") ? base : new URL(`${base.href}/`)), {
    method: "POST",
  ...
  const expectedHash = masumiInputHash(inputData);
  if (job.input_hash.toLowerCase() !== expectedHash) throw new Error("MASUMI_JOB_INPUT_HASH_MISMATCH");
...
export async function createMasumiPurchase(input: {
  network: MasumiNetwork;
  entry: MasumiVerifiedEntry;
  job: MasumiStartJob;
  inputData: unknown;
  amounts?: Array<{ amount: string; unit: string }>;
  metadata?: string;
}, config: MasumiPaymentConfig = masumiPaymentConfigFromEnv()): Promise<MasumiPurchase> {
  assertCardanoPaymentCredential(input.entry.sellerWallet.address, input.network, input.entry.sellerWallet.vkey);
  const request = {
    identifierFromPurchaser: input.job.identifierFromPurchaser,
    network: input.network,
    sellerVkey: input.entry.sellerWallet.vkey,
    paymentType: input.entry.paymentType ?? "Web3CardanoV1",
    blockchainIdentifier: input.job.blockchainIdentifier,
    payByTime: String(input.job.payByTime),
    submitResultTime: String(input.job.submitResultTime),
    unlockTime: String(input.job.unlockTime),
    externalDisputeUnlockTime: String(input.job.externalDisputeUnlockTime),
    agentIdentifier: input.entry.agentIdentifier,
    inputHash: masumiInputHash(input.inputData),
    ...(input.amounts?.length ? { Amounts: input.amounts } : {}),
    ...(input.metadata ? { metadata: input.metadata.slice(0, 2000) } : {}),
  };
  let payload: unknown;
  try {
    payload = await providerJson(`${config.baseUrl}/purchase`, config, { method: "POST", body: JSON.stringify(request) });
  } catch (error) {
    if (error instanceof TypeError || (error instanceof Error && error.name === "TimeoutError")) {
      const ambiguous = new Error("MASUMI_PURCHASE_SUBMISSION_UNKNOWN") as Error & { ambiguous?: boolean };
...
export async function findMasumiPurchase(network: MasumiNetwork, blockchainIdentifier: string, config: MasumiPaymentConfig = masumiPaymentConfigFromEnv()): Promise<MasumiPurchase | null> {
  ...
  for (let page = 0; page < config.maxLookupPages; page += 1) {
    const url = new URL(`${config.baseUrl}/purchase`);
    ...
    if (exact.length > 1) throw new Error("MASUMI_PURCHASE_AMBIGUOUS");
    if (exact[0]) return exact[0];
    if (parsed.data.Purchases.length < 100) return null;
    cursorId = parsed.data.Purchases.at(-1)?.id;
...
export async function fetchMasumiJobStatus(entry: MasumiVerifiedEntry, jobId: string): Promise<MasumiJobStatus> {
  const url = new URL("status", entry.apiBaseUrl.endsWith("/") ? entry.apiBaseUrl : `${entry.apiBaseUrl}/`);
  ...
  if (!response.ok) throw new Error(`MASUMI_AGENT_STATUS_${response.status}`);
...
export async function requestMasumiRefund(network: MasumiNetwork, blockchainIdentifier: string, config: MasumiPaymentConfig = masumiPaymentConfigFromEnv()) {
  ...
  if (parsed.status.toLowerCase() !== "success" || parsed.data.blockchainIdentifier !== blockchainIdentifier) throw new Error("MASUMI_REFUND_RESPONSE_INVALID");
  return parsed.data;
}
...
export async function authorizeMasumiRefund(network: MasumiNetwork, blockchainIdentifier: string, config: MasumiPaymentConfig = masumiPaymentConfigFromEnv()) {
  ...
  if (parsed.status.toLowerCase() !== "success" || parsed.data.blockchainIdentifier !== blockchainIdentifier) throw new Error("MASUMI_REFUND_AUTH_RESPONSE_INVALID");
  return parsed.data;
}
...
export function verifyMasumiResultHash(purchase: MasumiPurchase, status: MasumiJobStatus) {
  ...
  const expected = (purchase.resultHash ?? "").toLowerCase();
  if (!/^[0-9a-f]{64}$/.test(expected)) throw new Error("MASUMI_RESULT_HASH_UNAVAILABLE");
  const actual = masumiResultHash(status.result);
  if (actual !== expected) throw new Error("MASUMI_RESULT_HASH_MISMATCH");
  return { result: status.result, resultHash: actual };
}

export async function prepareMasumiEscrow(input: { agentIdentifier: string; network: MasumiNetwork; inputData: unknown }) {
  const entry = await fetchMasumiAgent(input.agentIdentifier, input.network);
  assertCardanoPaymentCredential(entry.sellerWallet.address, input.network, entry.sellerWallet.vkey);
  const job = await startMasumiJob(entry, input.inputData);
  return { entry, job };
}

export function masumiPaymentReadinessErrors(env: NodeJS.ProcessEnv = process.env): string[] {
  if (env.MASUMI_ESCROW_ENABLED !== "true") return [];
  try { masumiPaymentConfigFromEnv(env); return []; }
  catch (error) { return [error instanceof Error ? error.message : "MASUMI_PAYMENT_CONFIG_INVALID"]; }
}
