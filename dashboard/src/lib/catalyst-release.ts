...
export const catalystEvidenceTypes = ["CARDANO_PREPROD_ADA_CANARY","CARDANO_PREPROD_TOKEN_CANARY","CARDANO_MAINNET_USDCX_CANARY","PYTH_LIVE_FEEDS","MASUMI_REGISTRY_LIVE","MASUMI_ESCROW_COMPLETED","MASUMI_RESULT_HASH_VERIFIED","MASUMI_REFUND_DRILL","VERIDIAN_CREDENTIAL_VERIFIED","DUNE_PUBLISHED","DUNE_SAMPLE_VERIFIED","REMOTE_SIGNER_CUSTODY_REVIEW","MONITORING_ONCALL","PITR_RESTORE_DRILL","INCIDENT_EXERCISE","INDEPENDENT_SECURITY_ASSESSMENT"] as const;
...
export function catalystProductionConfigErrors(env: NodeJS.ProcessEnv = process.env) {
  ...
  const errors = [
    ...(env.APP_ENV === "production" ? [] : ["APP_ENV=production"]), ...(env.CARDANO_USDCX_ENABLED === "true" ? [] : ["CARDANO_USDCX_ENABLED=true"]),
    ...(env.PYTH_POLICY_ENABLED === "true" ? [] : ["PYTH_POLICY_ENABLED=true"]), ...(env.MASUMI_POLICY_ENABLED === "true" ? [] : ["MASUMI_POLICY_ENABLED=true"]),
    ...(env.MASUMI_ESCROW_ENABLED === "true" ? [] : ["MASUMI_ESCROW_ENABLED=true"]), ...(env.VERIDIAN_IDENTITY_ENABLED === "true" ? [] : ["VERIDIAN_IDENTITY_ENABLED=true"]),
    ...(env.DUNE_ANALYTICS_ENABLED === "true" ? [] : ["DUNE_ANALYTICS_ENABLED=true"]), ...cardanoAssetReadinessErrors(env), ...pythReadinessErrors(env), ...masumiReadinessErrors(env),
    ...masumiPaymentReadinessErrors(env), ...veridianReadinessErrors(env), ...duneReadinessErrors(env), ...releaseEvidenceAuthErrors(env),
  ];
  ...
  if (!env.CARDANO_MAINNET_PROVIDER_ADDRESS) errors.push("CARDANO_MAINNET_PROVIDER_ADDRESS");
...
function duneTransactionHash(row: Record<string, unknown>) {
  const value = row.transaction_hash;
...
export async function liveCatalystDependencyChecks() {
  ...
  if (!dune.sample?.rows.length) throw new Error("DUNE_VERIFICATION_SAMPLE_EMPTY");
  ...
  const hashes = dune.sample.rows.map(duneTransactionHash).filter((value): value is string => Boolean(value)).slice(0, 3);
  if (!hashes.length) throw new Error("DUNE_VERIFICATION_SAMPLE_INVALID");
  const verifiedSamples: string[] = [];
  for (const transactionHash of hashes) {
    const evidence = await cardanoTransactionEvidence("cardano:mainnet", transactionHash);
    if (!evidence || !evidence.validContract || !evidence.outputs.some((output) => output.address === provider)) throw new Error("DUNE_BLOCKFROST_SAMPLE_MISMATCH");
    verifiedSamples.push(transactionHash);
  }
  return {
...
export async function catalystReleaseEvidenceStatus(releaseSha = process.env.RELEASE_SHA ?? "") {
  ...
  const present = [...new Set(valid.map((row) => row.evidenceType))], missing = catalystEvidenceTypes.filter((type) => !present.includes(type));
