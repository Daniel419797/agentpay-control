...
const secretStep = z.object({
  op: z.literal("secret"),
  selector,
  secret: secretName,
}).strict();

const frameSecretStep = z.object({
  op: z.literal("frame_secret"),
...
export const checkoutPlanSchema = z.object({
  ...
  if (required.size) {
    ctx.addIssue({
      code: "custom",
      message: `Checkout plan is missing required card fields: ${[...required].join(", ")}`,
      path: ["steps"],
    });
  }
  ...
  if (plainBytes > 16 * 1024) {
    ctx.addIssue({ code: "custom", message: "Checkout plan contains too much non-sensitive form data.", path: ["steps"] });
  }
  if (!plan.successUrlPrefix && !plan.successText) {
    ctx.addIssue({ code: "custom", message: "Provide successUrlPrefix or successText so checkout completion can be verified.", path: [] });
  }
});
...
export function normalizeHostPattern(value: string) {
  ...
  if (!host || host.length > 253 || host.includes("/") || host.includes(":") || host.includes("@") || host.includes("..")) {
    throw new Error("CARD_AUTONOMY_HOST_INVALID");
  }
