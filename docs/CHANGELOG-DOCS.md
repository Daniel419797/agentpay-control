...
- Added clear disclosure of the project's original Hedera x402 bounty origin where proposal or provenance context requires it.
- Updated Cardano Mainnet architecture to external per-agent Ed25519 custody plus self custody.
- Clarified that the Cardano signer constructs and signs transactions but does not submit them.
...
- Replaced old Hedera-only requirements, design, screens, workflows, demo and testing documents with current-state versions while retaining the old revisions in Git history.
- Kept Catalyst maturity at **TRL 5** until a relevant-environment Mainnet and pilot demonstration exists.
- Explicitly separated source implementation from real deployment, provider and pilot evidence.
- Added Catalyst checklist language requiring internally consistent wallet, transaction, fee and adoption targets instead of treating planning numbers as repository achievements.

## Files reviewed

The pass covered the root project and security documentation, every Markdown document under `docs/`, Cardano signer documentation, Dune analytics documentation, the MCP package README, the agent integration `SKILL.md`, and documentation-facing environment comments relevant to the new Mainnet custody path.
