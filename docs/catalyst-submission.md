...
The earlier Catalyst narrative predated the completed Cardano Mainnet external per-agent custody path and identified the primary contributor only by GitHub handle. This version states the contributor identity and prior Hedera program involvement plainly, reflects the current code, and keeps TRL and adoption claims separate from unproven planning assumptions.

## Required disclosure

**Daniel Praise** (`Daniel419797`) is the repository owner and primary technical contributor.

AgentPay was originally built for the **Hedera x402 bounty** and later extended into the current multi-rail control plane. That prior program involvement is explicitly disclosed. Previously completed Hedera work is not presented as new Catalyst-funded delivery; Catalyst scope should cover the Cardano-specific and pilot work that remains.

## Current maturity

AgentPay is described conservatively as **TRL 5** for Catalyst purposes.

The repository now implements Cardano Mainnet external per-agent custody in addition to self custody. That removes the earlier source-code limitation under which Mainnet was unsigned/self-custody-only. Source implementation alone is not the same as a completed relevant-environment pilot or demonstration, so the code change by itself is not used to claim TRL 6.
...
- counterparty identity and trust;
- isolated payment identities and signing authority;
- exact resource and payment binding;
- replay protection;
- safe treatment of ambiguous submissions;
- escrow, result and refund evidence where direct payment is insufficient;
- private organizational policy off-chain with auditable public settlement evidence.

## Solution

AgentPay places a deterministic policy and trust layer between an autonomous agent and payment execution.
...
The implemented Mainnet path does **not** use a shared or deployment-wide managed-agent master key.
...
- signer constructs and signs but does not submit Cardano transactions;
- facilitator submits and confirms through Blockfrost;
- canonical resource SHA-256 binding;
- exact payer, payee, asset, amount, conservation and change validation;
...
- Pyth, Masumi and KERI required evidence fails closed;
...
Any demo presented as evidence must identify what is actually live in that exact environment.

Minimum Cardano evidence for a specific demonstrated profile should include:

1. exact release SHA;
2. deployed dashboard, facilitator and signer endpoints;
...
7. real Pyth, Masumi or KERIA configuration only if those integrations are shown as live;
8. real Dune query and dashboard IDs only if Dune is shown as published evidence.
...
Show the agent's immutable payment identity, custody mode and published financial and trust policy.

### 2. Counterparty trust

Show Masumi seller, capability and payment-key evidence and optional KERI evidence if actually configured.

### 3. Direct x402

Show 402 challenge, policy result, signing path, facilitator verification and submission, chain confirmation and paid resource response.
...
If Mainnet external custody is part of the environment, demonstrate or explain that an unavailable or invalid custody response blocks signing rather than falling back to a shared key.

### 6. Escrow

If real Masumi escrow is configured, show purchase, job and result evidence and a separate refund path where appropriate.
...
**Observed** metrics from actual stored or published evidence must be reported separately from **proposal targets**.
...
- policy-denied and approval-required counts;
- confirmation latency where recorded;
- verified Masumi completion, refund and dispute outcomes;
- public Dune sample cross-checks.

Customer counts, transaction volume, fees, revenue, uptime or adoption must not be inferred from synthetic fixtures.

Any Catalyst pilot target for wallets, transactions, fees or first-two-week adoption must be internally consistent with the actual acquisition model written in the proposal. Those planning commitments belong in the proposal and pilot plan; this repository does not invent them as completed facts.

## Suggested short pitch

AgentPay gives autonomous agents bounded financial authority instead of unrestricted wallets. On Cardano, AgentPay applies immutable policy, approvals and counterparty trust before payment. Preprod managed agents use isolated per-agent identities, while Mainnet can use self custody or a distinct external Ed25519 signer identity for each agent. AgentPay constructs the payment, verifies returned signatures locally, independently verifies the final Cardano transaction in the facilitator, submits through Blockfrost and reconciles chain evidence. For jobs needing stronger buyer protection, the same control plane can use Masumi escrow, with optional Pyth-valued budgets and KERI identity constraints.

## Production and readiness wording

A safe current statement is:

AgentPay has been deployed and exercised in supported environments. The repository implements policy-controlled Cardano x402, Preprod per-agent managed signing, Mainnet self custody and external per-agent Ed25519 custody, Pyth-valued policy, Masumi trust, escrow, refunds and reputation, optional KERI identity and Dune observability. Readiness for a specific production or pilot profile depends on the exact release and external configuration being operated.

For Catalyst purposes, the current maturity remains **TRL 5** until the intended Mainnet and pilot configuration is demonstrated in a relevant environment.

## Update summary

Updated 2026-08-22 to:

- identify Daniel Praise rather than only `Daniel419797`;
- disclose the Hedera x402 origin directly;
