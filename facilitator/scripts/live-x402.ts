...
function liveFacilitatorUrl(){const url=new URL(required("FACILITATOR_URL"));const local=["localhost","127.0.0.1","::1"].includes(url.hostname);if(url.protocol!=="https:"&&!local)throw new Error("FACILITATOR_URL must use HTTPS unless it targets localhost");return url;}
...
const facilitatorUrl=liveFacilitatorUrl();
const verified=await fetch(new URL("/verify",facilitatorUrl),{method:"POST",headers:{"content-type":"application/json"},body});
...
const settled=await fetch(new URL("/settle",facilitatorUrl),{method:"POST",headers:{"content-type":"application/json"},body});
