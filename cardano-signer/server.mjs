...
function validateAgentId(agentId){if(!AGENT_ID.test(agentId))throw new Error("CARDANO_MANAGED_AGENT_ID_INVALID");return agentId;}
...
function configFromEnv(){
  ...
  if(Boolean(cfg.agentCustodyUrl)!==Boolean(cfg.agentCustodyApiKey))throw new Error("CARDANO_AGENT_CUSTODY_CONFIG_INCOMPLETE");
  if(cfg.agentCustodyApiKey&&cfg.agentCustodyApiKey.length<32)throw new Error("CARDANO_AGENT_CUSTODY_API_KEY_TOO_SHORT");
  if(cfg.agentCustodyApiKey&&safeEqual(cfg.apiKey,cfg.agentCustodyApiKey))throw new Error("CARDANO_AGENT_CUSTODY_KEY_MUST_BE_DISTINCT");
  if(cfg.appEnv==="production"){
    ...
    if(cfg.agentCustodyUrl&&new URL(cfg.agentCustodyUrl).protocol!=="https:")throw new Error("CARDANO_AGENT_CUSTODY_URL_HTTPS_REQUIRED");
    if(cfg.signingMode==="managed"){
      ...
      if(cfg.agentCustodyApiKey&&safeEqual(cfg.remoteSignerApiKey,cfg.agentCustodyApiKey))throw new Error("CARDANO_AGENT_CUSTODY_KEY_MUST_BE_DISTINCT");
    }
...
function addressFromPublicKey(publicKey,network){if(!Buffer.isBuffer(publicKey)||publicKey.length!==32)throw new Error("CARDANO_AGENT_CUSTODY_PUBLIC_KEY_INVALID");const credential=blake2b(publicKey,28),networkId=network==="cardano:mainnet"?1:0,header=Buffer.from([(6<<4)|networkId]);return bech32Encode(networkId===1?"addr":"addr_test",Buffer.concat([header,credential]));}
function managedAgentSeed(cfg,agentId){if(cfg.network!=="cardano:preprod")throw new Error("CARDANO_MANAGED_AGENT_MASTER_KEY_TESTNET_ONLY");if(!cfg.agentMasterKey)throw new Error("CARDANO_MANAGED_AGENT_MASTER_KEY_REQUIRED");validateAgentId(agentId);return createHmac("sha256",cfg.agentMasterKey).update(`agentpay-managed-v1|${cfg.network}|${agentId}`).digest("hex");}
function preprodManagedAgentIdentity(cfg,agentId){const seed=managedAgentSeed(cfg,agentId),publicKey=publicKeyFromSeed(seed),address=addressFromPublicKey(publicKey,cfg.network);return{seed,publicKey,address,signerRef:`agent:${agentId}`};}

async function custodyJson(cfg,path,body){if(!cfg.agentCustodyUrl||!cfg.agentCustodyApiKey)throw new Error("CARDANO_AGENT_CUSTODY_REQUIRED");let response;try{response=await fetch(`${cfg.agentCustodyUrl}${path}`,{method:"POST",headers:{"content-type":"application/json",accept:"application/json",authorization:`Bearer ${cfg.agentCustodyApiKey}`},body:JSON.stringify(body),redirect:"error",signal:AbortSignal.timeout(15000)});}catch{throw new Error("CARDANO_AGENT_CUSTODY_UNAVAILABLE");}if(!response.ok)throw new Error(`CARDANO_AGENT_CUSTODY_${response.status}`);try{return await response.json();}catch{throw new Error("CARDANO_AGENT_CUSTODY_RESPONSE_INVALID");}}
async function externalManagedAgentIdentity(cfg,agentId){if(cfg.network!=="cardano:mainnet")throw new Error("CARDANO_AGENT_CUSTODY_MAINNET_ONLY");validateAgentId(agentId);const payload=await custodyJson(cfg,"/identity",{network:cfg.network,agentId,algorithm:"Ed25519",purpose:"cardano-payment"});const publicKeyHex=typeof payload?.publicKeyHex==="string"?payload.publicKeyHex:"",signerRef=typeof payload?.signerRef==="string"?payload.signerRef:"";if(!/^[0-9a-fA-F]{64}$/.test(publicKeyHex)||!signerRef||signerRef.length>256)throw new Error("CARDANO_AGENT_CUSTODY_IDENTITY_INVALID");const publicKey=Buffer.from(publicKeyHex,"hex"),address=addressFromPublicKey(publicKey,cfg.network),claimed=typeof payload?.payerAddress==="string"?payload.payerAddress:(typeof payload?.accountId==="string"?payload.accountId:undefined);if(claimed&&claimed!==address)throw new Error("CARDANO_AGENT_CUSTODY_ADDRESS_MISMATCH");return{publicKey,address,signerRef};}
async function managedAgentIdentity(cfg,agentId){return cfg.network==="cardano:preprod"?preprodManagedAgentIdentity(cfg,agentId):externalManagedAgentIdentity(cfg,agentId);}
...
async function signManagedAgentHash(cfg,agentId,identity,hash){if(identity.seed)return signHashWithSeed(identity.seed,hash);const payload=await custodyJson(cfg,"/sign",{network:cfg.network,agentId,signerRef:identity.signerRef,payerAddress:identity.address,algorithm:"Ed25519",messageHex:Buffer.from(hash).toString("hex"),purpose:"cardano-transaction-body"});const signatureHex=typeof payload?.signatureHex==="string"?payload.signatureHex:"";if(!/^[0-9a-fA-F]{128}$/.test(signatureHex))throw new Error("CARDANO_AGENT_CUSTODY_SIGNATURE_INVALID");if(typeof payload?.signerRef==="string"&&payload.signerRef!==identity.signerRef)throw new Error("CARDANO_AGENT_CUSTODY_SIGNER_MISMATCH");if(typeof payload?.publicKeyHex==="string"&&payload.publicKeyHex.toLowerCase()!==identity.publicKey.toString("hex"))throw new Error("CARDANO_AGENT_CUSTODY_PUBLIC_KEY_MISMATCH");const signature=Buffer.from(signatureHex,"hex");if(!verifyEd25519(identity.publicKey,hash,signature))throw new Error("CARDANO_AGENT_CUSTODY_SIGNATURE_INVALID");return signature;}
  ...
  return createServer(async(req,res)=>{
    try{
      ...
      if(req.url==="/managed-identity"){
        const agentId=String(body.agentId??"");if(body.network!==cfg.network)throw new Error("CARDANO_NETWORK_MISMATCH");const identity=await managedAgentIdentity(cfg,agentId);return json(res,200,{accountId:identity.address,payerAddress:identity.address,publicKey:identity.publicKey.toString("hex"),signerRef:identity.signerRef});
      }
