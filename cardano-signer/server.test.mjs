...
function withEnv(values,fn){const before={...process.env};for(const key of Object.keys(process.env))delete process.env[key];Object.assign(process.env,values);try{return fn();}finally{for(const key of Object.keys(process.env))delete process.env[key];Object.assign(process.env,before);}}
...
test("production rejects a payer address that does not belong to the configured public key",()=>withEnv({...base,CARDANO_PAYER_ADDRESS:address(Buffer.alloc(32,7))},()=>assert.throws(()=>configFromEnv(),/PAYER_KEY_MISMATCH/)));
test("invalid USDCx policy-id plus asset-name configuration fails startup",()=>withEnv({...base,CARDANO_USDCX_ASSET_ID:"usdcx"},()=>assert.throws(()=>configFromEnv(),/ASSET_UNIT_INVALID/)));
test("complete ADA and optional USDCx production configuration is accepted",()=>withEnv({...base,CARDANO_USDCX_ASSET_ID:USDCX},()=>{const cfg=configFromEnv();assert.equal(cfg.network,"cardano:preprod");assert.equal(cfg.usdcxAssetId,USDCX);}));
