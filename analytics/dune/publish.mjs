...
function render(sql, providerAddress, assetUnit) {
  return sql
    .replaceAll("'{{provider_address}}'", sqlLiteral(providerAddress))
    .replaceAll("'{{usdcx_asset_unit}}'", sqlLiteral(assetUnit));
}
...
async function publishOne({ file, name, description, existingId }) {
  const providerAddress = required("DUNE_PROVIDER_ADDRESS").toLowerCase();
  const assetUnit = required("DUNE_USDCX_ASSET_UNIT").toLowerCase();
  ...
  const template = await readFile(join(here, file), "utf8");
  const querySql = render(template, providerAddress, assetUnit);
...
const overview = await publishOne({
  file: "agentpay_cardano_overview.sql",
  name: "AgentPay Cardano x402 Overview",
  description: "Public on-chain AgentPay Cardano x402 settlement activity. Contains no private AgentPay application data.",
  existingId: process.env.DUNE_AGENTPAY_OVERVIEW_QUERY_ID,
});
const activity = await publishOne({
  file: "agentpay_cardano_activity.sql",
  name: "AgentPay Cardano x402 Daily Activity",
  description: "Daily public Cardano transaction activity for the configured AgentPay provider address.",
  existingId: process.env.DUNE_AGENTPAY_ACTIVITY_QUERY_ID,
});

console.log(JSON.stringify({ overview, activity }, null, 2));
